        ...
        <aside class="right-side" style="margin-left:-26px;">
            ...
            <!-- Main content -->
                ...
                <div class="row">
					...
					<div class="date">
							 {{  Form::open(array('action'=>'JoshController@alltransactions', 'id'=>'addcompa','method' => 'post','files' => 'true')) }}
...
<!--				<div class="num-item trans"> 
					{{ Form::open(array('method' => 'GET','url' => 'admin/transaction')) }}
				...
				@foreach($result as $userrs=>$userr)
				
<tr>
						<td>{{ $userr->order_date }}</td>
						<td><a href="{{ URL::to('admin/transaction_detail/' .$userr->id) }}">{{ $userr->customer  }}</a></td>
						
						<td>{{ $userr->city }}</td>
						<td>{{ $userr->customer_state }}</td>
						<td>{{ $userr->service }}</td>
						<td>${{ $userr->total_price }}</td>
                        
						 <td>{{ $userr->workername }}</td>
						  <td>{{ $userr->customer_rate }}</td>
</tr>
@endforeach
...
<script language="javascript" type="text/javascript" src="{{ asset('assets/vendors/flotchart/js/jquery.flot.selection.js') }}"></script>
	...
	<script language="javascript" type="text/javascript" src="{{ asset('assets/js/pages/customcharts.js') }}"></script>
		<script src="{{ asset('assets/js/pages/transactioncharts.js') }}"></script>
<script language="javascript" type="text/javascript" src="{{ asset('assets/js/pages/noconflict.js') }}"></script>
...
//	$(".sidebar").height($('.below-four').height());
	draw();
});
$(".sidebar").height($(".graph-cntnr").height() + $(".dataTables_wrapper").height());
    </script>
	<script>
